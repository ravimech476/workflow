<template>
    <f7-page>
        <f7-navbar :title="`${trans('modules.projects.note')} | ${trans('app.task')} #${item.task_short_code}` ">
            <f7-nav-right>
...
<!--                    </template>-->
                ...
                <f7-block v-if="!busy && item.notes.length === 0" class="text-align-center">
                    {{ trans('messages.noNoteFound') }}
                </f7-block>
            </f7-list>
...
export default {
    ...
    methods: {
        ...
        remove(item) {
            f7.preloader.show();
            axiosHttp.delete(`/task/${this.item.id}/note/${item.id}`).then(
                (response) => {
        ...
        removeFromList(subtask) {
            const index = this.item.notes.indexOf(subtask);
