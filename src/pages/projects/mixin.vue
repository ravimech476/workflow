    ...
    methods: {
        ...
        getItem() {
            ...
            const fields = [
                ...
                'total_earnings',
                'tasks{id,heading}',
                'client{id,name,email,image,image_url}',
                'client:client_details{id,company_name,address}',
                'members:user{id,name,email,image,image_url}',
                'members:user:employeeDetail.limit(1000){id,employee_id}',
                'members:user:employeeDetail:department.limit(1000){id,team_name}',
                'members:user:employeeDetail:designation.limit(1000){id,name}',
                'hours_logged',
                'category',
                'currency.limit(1000){id,currency_code,currency_symbol}'
            ];

            const encodedFields = encodeURIComponent(fields.join(','));

            return Cache.httpGet(`/project/${this.f7route.params.id}?fields=${encodedFields}`)
                .then(response => {
